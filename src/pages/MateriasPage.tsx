...
import { eliminarMateria, listarMaterias } from "../api/materiaApi";
...
export default function MateriasPage() {
  ...
  const [materiaEliminar, setMateriaEliminar] = useState<{ id: number; nombre: string } | null>(null);
  ...
  // Abrir modal de confirmación
    const abrirModalEliminar = (materia: Materia) => {
      if (materia.id) {
        setMateriaEliminar({
          id: materia.id,
  ...
  // Confirmar eliminación
    const confirmarEliminar = async () => {
      ...
      try {
        ...
        await eliminarMateria(materiaEliminar.id);
        
        // Optimistic update
        setMaterias(prev => prev.filter(materia => materia.id !== materiaEliminar.id));
        
        // Mostrar toast de éxito
        mostrarToast(
          `Materia "${materiaEliminar.nombre}" eliminada exitosamente`,
          'success'
        );
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
          ...
          <ModalConfirmacion
            ...
            mensaje={
              materiaEliminar 
                ? `¿Estás seguro de eliminar la materia "${materiaEliminar.nombre}"?\n\nEsta acción no se puede deshacer.`
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Total: <span className="font-semibold">{materias.length}</span> materias registradas
              </p>
            </div>
            
            <div className="mt-4 md:mt-0">
              <Link to="/materias/crear">
                    ...
                    {materias.map((materia) => (
                      ...
                      >
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-mono text-gray-500">#{materia.id}</span>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex space-x-2">
                            <Link to={`/materias/editar/${materia.id}`}>
                              <button 
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <div className="flex flex-col sm:flex-row justify-between items-center">
                  <div className="text-sm text-gray-700 mb-2 sm:mb-0">
                    Mostrando <span className="font-semibold">{materias.length}</span> materias
                  </div>
                  <div className="text-sm text-gray-600">
                    Haz clic en los botones de acción para gestionar cada materia
                  </div>
                </div>
              </div>
            </div>
